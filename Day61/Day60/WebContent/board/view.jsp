...
<article class="board">
	<table class="boards">
		...
		<c:set var="goodCnt" value="${dao.getGood(param.idx, 1) }"/>
		<c:set var="badCnt" value="${dao.getBad(param.idx, 2) }"/>
		
		<tr>
			<td colspan="6">
				<div class="recommend">
				<c:set var="args" value="board_idx=${param.idx }&nick=${user.nick }&isCheck=${isCheck }"/>
					<a href="recommend/recPro.jsp?${args }&good=1">
						<button class="${isCheck == 1 ? 'good' : ''}">
							추천<br>
							(${goodCnt })
						</button>
					</a>
					<a href="recommend/recPro.jsp?${args }&good=2">
